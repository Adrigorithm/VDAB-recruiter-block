...
Most things should work now. The initial release should be available on the [firefox addons page](https://addons.mozilla.org/en-GB/firefox/extensions/) and probably chromium as well by the time you're reading this.

Feel free to report any problems which may occur.
...
This works for both your personal job suggestions page and the general job search page.
...
The script targets the `.c-vacature-container` element on VDAB vacancy pages. Upon detecting a change or initial load, it:

1. **Polls for Stability:** Waits until the vacancy lists stops loading new items to avoid race conditions.
...
* **Manual Updates Required:** To block new agencies, you must manually add them using the popup menu.
